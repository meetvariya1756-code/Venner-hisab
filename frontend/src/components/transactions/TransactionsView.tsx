    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Review, categorize, and export normalized transactions</p>
        </div>
                ...
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4 max-w-sm">
                      <div className="font-medium text-white truncate" title={tx.narration}>
                        {tx.narration}
                      </div>
                      <div className="text-[10px] text-slate-500 mt-0.5 flex items-center gap-2">
                        <span>Stmt Pg {tx.page_number}</span>
                        {tx.party_name && (
                          <span className="bg-slate-800 px-1.5 py-0.5 rounded text-slate-300 font-semibold">
                            Party: {tx.party_name}
                          </span>
                        )}
                      </div>
                    ...
                    <td className="py-3 px-4">
                      ...
                      >
                        <option value="" className="bg-slate-900 text-slate-400">Uncategorized</option>
                        {categories.map((c) => (
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-card w-full max-w-md p-6 space-y-4">
            ...
            <form onSubmit={handleSaveRule} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Narration Pattern (Keyword)</label>
                <input
                  type="text"
                  value={rulePattern}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Associate Party (Optional)</label>
                <select
                  value={rulePartyId}

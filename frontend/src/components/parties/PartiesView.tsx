    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Consolidate vendors, buyers, and marketplace payment entities</p>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-indigo-400 font-bold text-sm">
                      {p.name.charAt(0)}
                    </div>
                    <div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-200">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {aliasesList.map((alias, idx) => (
                        <span key={idx} className="text-[11px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-mono">
                          {alias}
                        </span>
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-card w-full max-w-md p-6 space-y-4">
            ...
            <form onSubmit={handleMerge} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Primary Target Party</label>
                <select
                  value={targetPartyId}
